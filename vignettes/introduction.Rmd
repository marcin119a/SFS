...
\begin{itemize}
\item \texttt{sampleSFS} is the main function that can find the SFS from an inital NMF
\item \texttt{NMFPois} can find an initial NMF solution from a data matrix
\item \texttt{gkl.dev} is an internal function for \texttt{NMFPois}, that calculates the generalized Kullback-Leibler
\item \texttt{plotSFS} will plot the SFS  
\item \texttt{samplesToSVD} will transform SFS solutions from \texttt{sampleSFS} relative to SVD solution
\end{itemize}
...
inital.fit$P
initial.fit$E
inital.fit$P%*%initial.fit$E #approximation of M
```

Now, as an initial solution has been constructed one can find the $SFS$ with the function \texttt{sampleSFS}. 
